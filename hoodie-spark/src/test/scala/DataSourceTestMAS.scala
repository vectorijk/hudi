...
class HudiDataSourceTest extends AssertionsForJUnit {
  ...
  @Test def testOnlyP1() {
    ...
    val col24: String =
      s"""{"timestamp": 5.0, "_row_key": "${uuid1}", "name": "p1", "location": 17, "partition": "2015/03/16", "history": []}"""
    val col25: String =
      s"""{"timestamp": 6.0, "_row_key": "${uuid1}", "name": "p1", "location": 18, "partition": "2015/03/16", "history": []}"""
    val col26: String =
      s"""{"timestamp": 7.0, "_row_key": "${uuid1}", "name": "p1", "location": 20, "partition": "2015/03/16", "history": []}"""
    val col27: String =
      s"""{"timestamp": 8.0, "_row_key": "${uuid1}", "name": "p1", "location": 21, "partition": "2015/03/16", "history": []}"""
    val col28: String =
      s"""{"timestamp": 9.0, "_row_key": "${uuid1}", "name": "p1", "location": 22, "partition": "2015/03/16", "history": []}"""

    val tb2 = List(col21, col22, col23, col24, col25, col26, col27)
    //    val tb2 = List(col21, col24, col25)
    ...
    val result_df = hoodieROViewDF2.sort("name").collect().toSeq
    assertEquals("WrappedArray(4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1)",
      result_df(0).getAs[List[String]]("history").toString)
    //    assertEquals("WrappedArray(3.0#11, 2.0#14, 1.0#5)",
    //      result_df(1).getAs[List[String]]("history").toString)
  }
  ...
  @Test def testCopyOnWriteStorage() {
    ...
    //    +--------------------------------------+--------+----+----------+---------+
    //    |[3.0#11, 2.0#14, 1.0#5]               |11      |p2  |2015/03/16|3.0      |
    //    |[4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1]|16      |p1  |2015/03/16|4.0      |
    //    +--------------------------------------+--------+----+----------+---------+
  }
  ...
  @Test def testCopyOnWrite3CommitsStorage() {
    ...
    val col40: String =
      s"""{"timestamp": 6.5, "_row_key": "${uuid1}", "name": "p1", "location": 32, "partition": "2015/03/16", "history": []}"""
    val col41: String =
    ...
    val result_df = hoodieROViewDF4.sort("name").collect().toSeq
    // p1
    assertEquals(
      "WrappedArray(10.0#40, 9.0#39, 8.0#33, 7.0#34, 6.5#32, 6.0#20, 5.0#22, 4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1)",
      result_df(0).getAs[List[String]]("history").toString
    )
    // p2
    assertEquals("WrappedArray(8.0#37, 7.0#35, 6.0#23, 5.0#21, 3.0#11, 2.0#14, 0.0#2)",
      result_df(1).getAs[List[String]]("history").toString)
    //    +---------------------------------------------------------------------------------------+--------+----+----------+---------+
    //    |history                                                                                |location|name|partition |timestamp|
    //    +---------------------------------------------------------------------------------------+--------+----+----------+---------+
    //    |[10.0#40, 9.0#39, 9.0#39, 8.0#33, 6.0#20, 5.0#22, 4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1]|39      |p1  |2015/03/16|9.0      |
    //    |[8.0#37, 7.0#35, 6.0#23, 5.0#21, 3.0#11, 0.0#2]                                        |37      |p2  |2015/03/16|8.0      |
    //    +---------------------------------------------------------------------------------------+--------+----+----------+---------+
  }
  ...
  @Test def testCOWHistoryList() {
    ...
    val col45: String =
      s"""{"timestamp": 9.0, "_row_key": "${uuid1}", "name": "p1", "location": 39, "partition": "2015/03/16", "history": []}"""
    val col46: String =
      s"""{"timestamp": 9.7, "_row_key": "${uuid1}", "name": "p1", "location": 40, "partition": "2015/03/16", "history": []}"""
    val col43: String =
      s"""{"timestamp": 7.0, "_row_key": "${uuid2}", "name": "p2", "location": 35, "partition": "2015/03/16", "history": []}"""
    val col44: String =
      s"""{"timestamp": 8.0, "_row_key": "${uuid2}", "name": "p2", "location": 37, "partition": "2015/03/16", "history": []}"""
    ...
    val result_df = hoodieROViewDF4.sort("name").collect().toSeq
    assertEquals(
      "WrappedArray(50.0#20, 9.7#40, 9.0#39, 7.0#34, 6.0#22, 4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1)",
      result_df(0).getAs[List[String]]("history").toString
    )
    assertEquals("WrappedArray(8.0#37, 7.0#35, 6.0#23, 5.0#2, 3.0#11, 2.0#14)",
      result_df(1).getAs[List[String]]("history").toString)
    assertEquals("WrappedArray(45.0#21, 8.0#33)",
      result_df(2).getAs[List[String]]("history").toString)

    //    +------------------------------------------------------------------------+--------+----+----------+---------+
    //    |history                                                                 |location|name|partition |timestamp|
    //    +------------------------------------------------------------------------+--------+----+----------+---------+
    //    |[50.0#20, 10.0#40, 9.0#39, 7.0#34, 4.0#16, 3.0#15, 2.0#12, 1.0#3, 0.0#1]|39      |p1  |2015/03/16|9.0      |
    //    |[8.0#37, 7.0#35, 6.0#23, 5.0#2, 3.0#11, 2.0#14]                         |37      |p2  |2015/03/16|8.0      |
    //    |[45.0#21, 8.0#33]                                                       |33      |p3  |2015/03/16|8.0      |
    //    +------------------------------------------------------------------------+--------+----+----------+---------+
  }
  @Test def testCOWSingleElementList() {
    ...
    val result_df = hoodieROViewDF4.sort("name").collect().toSeq
    assertEquals("WrappedArray(50.0#20, 7.0#34, 3.0#11, 2.0#12, 0.0#1)",
      result_df(0).getAs[List[String]]("history").toString)
    assertEquals("WrappedArray(8.0#37, 6.0#23, 5.0#2)",
      result_df(1).getAs[List[String]]("history").toString)

    //    +----------------------------------------+--------+----+----------+---------+
    //    |history                                 |location|name|partition |timestamp|
    //    +----------------------------------------+--------+----+----------+---------+
    //    |[50.0#20, 7.0#34, 3.0#11, 2.0#12, 0.0#1]|34      |p1  |2015/03/16|7.0      |
    //    |[8.0#37, 6.0#23, 5.0#2]                 |37      |p2  |2015/03/16|8.0      |
    //    +----------------------------------------+--------+----+----------+---------+
  }
}
